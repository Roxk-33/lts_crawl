...
const getImg = async content => {
  ...
  // const patt1 = /http://www.kesum.cn>/gi;
  if (patt.test(content)) {
    content = await content.replace(patt, (match, capture) => {
      ...
      //图片不存在，替换成这个
      let result = '<img src="/images/error.jpg" style="width:400px;">';
      downImg(capture, "/Users/hao/Desktop/lts_node/static" + path)
        .then(() => {
          result = '<img src="' + path + '" style="width:400px;">';
        })
        .catch(e => {
          result = '<img src="/images/error.jpg" style="width:400px;">';
        });
      i = 0;
      while (i < 10000) {
        i++;
